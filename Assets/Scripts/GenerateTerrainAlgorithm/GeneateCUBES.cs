...
public class GeneateCUBES : MonoBehaviour
{
    public int depth;
    public int width;
    public int height;
    ...
    /// <summary>
    /// Ïðîñòî ñïîñîá õðàíåíèÿ ïîçèöèè âñåõ âåêòîðîâ
    /// </summary>
    ...
    private void InitiateGameObjects()
    {
        ...
        {
            float sample = GetPerlinNoise3D(vector, frequency) * scale + offSet;
            if(sample < 0.05f && sample > -0.05f)
            {
                sample = (sample + 1) / 2;
